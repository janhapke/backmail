...
- [x] **REPO-01**: User can run `backmail init [path]` to create a backmail repository at a given path (defaults to current directory); creates `.backmail/config.json`, `.backmail/log`, `archive/` (git repo), and `worktrees/`
- [x] **REPO-02**: `init` prompts interactively for any parameters not supplied as CLI flags: repo path, IMAP host, port, username, TLS, and password
- [x] **REPO-03**: `init` stores the password in the OS keyring and writes `passwordRef` to `.backmail/config.json`
- [x] **REPO-04**: `init` is non-destructive — refuses to overwrite an existing repo without `--force`
- [x] **REPO-05**: `init` detects non-TTY context (CI/piped stdin) and requires all parameters as CLI flags in that case

### Repository Discovery

- [x] **DISC-01**: All commands auto-detect the backmail repository by walking up from CWD looking for `.backmail/`
- [x] **DISC-02**: `--workdir <path>` global flag overrides auto-detection for all commands
- [x] **DISC-03**: Commands print a clear error when not inside a backmail repo and no `--workdir` is given

### Credentials

- [x] **CRED-01**: Config `passwordRef` field supports format `keyring:service=backmail;account=<username>`
- [x] **CRED-02**: `BACKMAIL_PASSWORD` env var accepted as password fallback when keyring is unavailable
- [x] **CRED-03**: Keyring unavailability (headless/CI) is handled gracefully — falls back to env var without crashing

### Existing Commands

- [x] **CMD-01**: All existing commands (sync, log, ls, view, checkout, restore) work correctly with the new repository structure and path derivation (`archive/` is the git repo)
- [x] **CMD-02**: `--account` flag and global account registry removed from all commands; no central config file
...
| REPO-03 | Phase 9 | Complete |
| REPO-04 | Phase 9 | Complete |
| REPO-05 | Phase 9 | Complete |
| DISC-01 | Phase 7 | Complete |
| DISC-02 | Phase 7 | Complete |
| DISC-03 | Phase 7 | Complete |
| CRED-01 | Phase 6 | Complete |
| CRED-02 | Phase 6 | Complete |
| CRED-03 | Phase 6 | Complete |
...
*Last updated: 2026-05-04 — v1.1 complete, all 13 requirements satisfied*
