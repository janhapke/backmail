...
- **Global scope**: `/home/jan/dev/backmail/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/jan/dev/backmail/.claude/notes/`)
...
2. Glob `/home/jan/dev/backmail/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/jan/dev/backmail/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/jan/dev/backmail/.claude/notes/` — works in any directory
